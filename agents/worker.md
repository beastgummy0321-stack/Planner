...
4. UI issue: the direction is settled — the feature's Decisions and the prototype whose path `attach` printed say what it looks like (it is outside your worktree: read it, never copy it in). When the backend is not ready, the issue names the fake adapter to build against: fake data lives behind that boundary only, never as fixtures inside components, so the wiring issue swaps one file. Before reporting a UI issue, look at it yourself: render the screen with fake data from a throwaway script kept OUTSIDE the repo (your temp dir), on a port nobody else uses, and compare with the reference the issue names — a layout you never saw is a review round you will lose. Never add a file to the repo for that. Installed UI skills are for execution quality only: the project's component kit (e.g. `shadcn/ui`) and at most one `impeccable` critique or polish pass before you report. Never a skill that picks a direction (`ui-ux-pro-max`, `taste-skill`); skip silently when not installed.
5. Run the verify commands. Report their real output. Never edit a test to make it pass, never assert current broken behaviour. Green is not done: break each Done rule your tests cover in memory only (a mock or monkeypatch, never a file edit) and report which test went red — a rule no test catches is unfinished work, not an open concern.
