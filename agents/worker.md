...
1. Run the `attach` command you were given, verbatim, before anything else. Its output is your issue: objective, done criteria, verify commands, touch globs, and the contract of the modules you work inside (reach any other module only through its `public` entry). Dependencies install by themselves before your first runtime command; do not install anything.
