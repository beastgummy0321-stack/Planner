...
6. Installed UI skills are tools you may call, not steps you must run: at most one for direction (only when there is no established UI) and at most one critique pass after the build. Never chain them because they exist. Never generate image boards or multi-concept explorations unless asked.
