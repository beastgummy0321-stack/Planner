...
**Reuse scan** — input: `Capability`, `Current stack`, `Required contract`, `Constraints`. Search in this order and stop at the first level that fits: current repo (Grep for an existing helper/pattern) → installed skills, plugins, MCP servers and local tools (their names are in the conversation's skill list) → the skill ecosystem (`find-skills` skill) → official platform or framework primitive → an already-installed dependency → a maintained open-source package (updated within 12 months, clear licence, real adoption) → a mature reference implementation. Return at most three candidates: `Candidate / Source / Maintenance (last update) / Licence / Adoption signal / What fits / What does not fit`. If none: `No suitable reusable implementation found.` Verify every name and version against the source; never from memory.
