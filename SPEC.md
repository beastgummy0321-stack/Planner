...
| a challenger that timed out counts as a review; a background Agent's PostToolUse fires at launch, so the first "CLEAR" in the launch notice became the verdict (1.3.2) | the receipt has `completed` + `verdict`, **hook-signed by the challenger's own `harness challenge <CLEAR\|CHALLENGE>`** (gate.mjs, agent_type challenger; its only allowed Bash); PostToolUse only falls back to a synchronous return's verdict line and ignores launch notices; `mode work` (plan→work only) refuses a missing verdict and a CLEAR whose `plan_hash` no longer matches with `Independent Challenge needed`; the mode stays plan and /crank itself re-dispatches the challenger on the draft on disk (one round), so the user only ever types /dig, /carve, /crank | `gate.mjs` challenger branch, `post.mjs` `afterAgent`, `harness challenge`, `harness mode`, crank skill (21, 22, 44) |
