...
| a challenger that timed out counts as a review; a background Agent's PostToolUse fires at launch, so the first "CLEAR" in the launch notice became the verdict (1.3.2) | the receipt has `completed` + `verdict`, **hook-signed by the challenger's own `harness challenge <CLEAR\|CHALLENGE>`** (gate.mjs, agent_type challenger; its only allowed Bash); PostToolUse only falls back to a synchronous return's verdict line and ignores launch notices; `mode work` (plan→work only) refuses a missing verdict and a CLEAR whose `plan_hash` no longer matches; `/carve challenge` re-dispatches on the existing draft | `gate.mjs` challenger branch, `post.mjs` `afterAgent`, `harness challenge`, `harness mode` (21, 22, 44) |
