...
import { toolKind, writeTargets, commandOf } from '../lib/host.mjs';
...
if (tool === 'shell') afterBash();
else if (tool === 'write' || tool === 'patch') afterWrite();
process.exit(0);
...
function afterBash() {
  ...
  lease.violations = [...(lease.violations || []), ...bad.map((f) => ({ file: f, tool_use_id: id, command: commandOf(input) }))];
...
function afterWrite() {
  let targets;
  try { targets = writeTargets(input); } catch (e) { fail(e.message); }
