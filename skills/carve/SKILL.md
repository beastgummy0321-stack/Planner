...
Write `.work/ready/F01-I01.md` with `id, feature, after[], touch[], do_not_touch[], verify[], privileged[], review, interface_change` and Objective, Done, Verify, Blocked if sections. Optional `deps[], legacy_migration, group` describe actual changes. Include affected tests in touch. Shared checker/typecheck/build belong to the manifest, not copied into every issue. Feature verify adds only checks not already covered by the full project suite. Use `review: planner` for material public-interface, schema, permission, money, ownership or dependency risks. A worker must not have to invent an unresolved product or interface decision. Split at meaningful ownership, dependency, rollback or verification boundaries; same context and verification usually stays together.
...
Run validate once after the coherent plan is written. Request one independent challenge, when delegation is authorized, only for unresolved high-impact assumptions; ordinary review flags do not automatically require it. Share the concise outcome, scope and unresolved decisions. Before the first issue, present the outcome, scope, issue sequence and verification once. Continue with authorize/start/crank only when that concrete plan is already approved or the user explicitly waived this checkpoint, and no material question remains. General permission to build does not approve an unseen plan; an unchanged approved plan needs no renewed confirmation. Otherwise stop at the requested planning boundary. Delete discovery only after its useful content is preserved.

Before returning, apply the shared contract’s Stage closeout once to this task’s changed documents; do not run a repository-wide documentation sweep.
