...
Run validate once after the coherent plan is written. Request one independent challenge, when delegation is authorized, only for unresolved high-impact assumptions; ordinary review flags do not automatically require it. Share the concise outcome, scope and unresolved decisions. If implementation is already authorized and no material question remains, record authorize, start the feature, and continue with crank in the same turn. Otherwise stop at the requested planning boundary. Delete discovery only after its useful content is preserved.
