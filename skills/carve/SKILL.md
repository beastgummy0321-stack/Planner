...
3. **Container checker.** Run `harness adapter plan`. If it says `up to date`, run `harness adapter check` only — nothing to approve, apply or prove. Otherwise return its install/modify/add list to the main conversation, which asks the user once; on approval `harness adapter apply --approved`, then `harness adapter check` (green) and `harness adapter prove` (red on a deliberate violation). Unsupported stack or resource kind → stop with `unsupported architecture adapter` / `unsupported ownership adapter`; never downgrade to "manual review".
