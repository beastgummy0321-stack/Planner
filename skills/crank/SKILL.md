...
4. For `review: planner`, review the current diff and record `review <id> approve`; a changed head invalidates that receipt. Merge only after required checks pass. Before a merge or main-tree re-plan, wait for in-flight worker shell calls to finish; do useful read-only work while waiting. A failed merge/integration check is investigated using its evidence. For an unchanged-environment integration retry, use `integrate feature <id> --resume`; after dependency/service changes or uncertainty, run fresh. Read [SPEC.md verification semantics](../../SPEC.md) only when diagnosing reuse/invalidation.
