...
5. If `review: planner`: dispatch `harness:planner` with the issue file, `harness diff <id>` and the CLI path; on approve the planner itself runs `harness review <id> approve` (hook-signed receipt for the current head); then `harness merge <id>`. Block → `harness block <id> "<reason>"`. Otherwise `harness merge <id>` directly. Merge runs the integration checker and, when the manifest declares `verify.smoke`, the runtime smoke in the issue's worktree at the merge commit (never in the main tree); red rolls the merge back and blocks the issue.
