...
export function statusText(project) {
  ...
  for (const f of features) {
    ...
    const decisions = section(f.body, 'Decisions'); if (decisions.length) { lines.push('Decisions:', ...decisions.slice(0, 6).map(d => (d.startsWith('-') ? d : `- ${d}`).slice(0, 300))); if (decisions.length > 6) lines.push(`More decisions: .work/features/${f.id}.md (read if relevant)`); }
