...
export function statusText(project) {
  const root = process.env.PLUGIN_ROOT || process.env.CLAUDE_PLUGIN_ROOT || path.resolve(import.meta.dirname, '..');
  const cli = `node "${root.replace(/\\/g, '/')}/bin/harness.mjs"`;
  ...
  for (const f of features) {
    ...
    const focused = leases.some(l => l.issue?.startsWith(f.id + '-')) || (f.data.branch && f.data.branch === branch);
    if (focused) {
      const outcome = section(f.body, 'Outcome').join(' ');
      if (outcome) lines.push(`Outcome: ${outcome.length > 600 ? outcome.slice(0, 600) + '… (full context in feature file)' : outcome}`);
      const decisions = section(f.body, 'Decisions');
      if (decisions.length) lines.push('Decisions:', ...decisions.slice(0, 3).map(d => (d.startsWith('-') ? d : `- ${d}`).slice(0, 200)));
      if (decisions.length > 3 || outcome.length > 600) lines.push(`More context: .work/features/${f.id}.md (read if relevant)`);
    } else lines.push(`Context: .work/features/${f.id}.md (read only when relevant)`);
    lines.push(`Closure: open (not closed); human acceptance items: ${f.data.human?.length || 0}; queue completion alone is not feature closure.`);
  ...
  lines.push(`Last interruption: ${interruptions.length ? interruptions.join('; ') + ' — inspect host liveness before any recovery' : 'none'}`);
  return lines.join('\n');
}
...
export function handoffCheck(project, sessionId) {
  ...
  return `Handoff check: ${s.compactions} automatic compactions this session. At a safe point with a clear next step and a concrete gain, optionally append one short handoff suggestion after the outcome; mid-change, same stage, pure Q&A, or already declined: stay silent.`;
}
