...
export function statusText(project) {
  ...
  lines.push(s.mode === 'grill' ? 'No-Build gate active: only .harness/scratch/** is writable; only the user ends /dig by typing /carve.'
    : s.mode === 'plan' ? 'plan mode: ARCHITECTURE.md and .work/** writable; /crank when issues are ready.'
    : 'work mode: main tree is orchestration only; implementation runs in claimed worktrees via /crank.');
  return lines.join('\n');
}
