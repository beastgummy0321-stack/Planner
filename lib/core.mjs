...
export function issueFingerprint(issueText, manifestText = '', source = '') {
  const body = issueText.replace(/\n\n## Blocked \([^)]*\)[\s\S]*$/, '');
  const fm = (/^---\r?\n([\s\S]*?)\r?\n---/.exec(manifestText) || [])[1] || '';
  return crypto.createHash('sha1').update(body).update('\0').update(fm).update('\0').update(source).digest('hex');
}
...
// Exclude queue bookkeeping from retry identity; source/dependency/contract changes matter.
export function sourceFingerprint(root) {
  const files = gitRaw(root, 'ls-files', '-z').split('\0').filter(f => f && !f.startsWith('.work/') && !f.startsWith('.harness/'));
  const hash = crypto.createHash('sha256');
  for (const f of files.sort()) { hash.update(f); try { hash.update(fs.readFileSync(path.join(root, f))); } catch { hash.update('deleted'); } }
  return hash.digest('hex');
}
export function assertNoWorkerCommands(p) {
  const dir = path.join(p.harness, 'runtime', 'baselines');
  if (fs.existsSync(dir) && fs.readdirSync(dir).some(f => f.endsWith('.json'))) throw new Error('worker command still in flight: wait for its result before changing the main tree');
}
