...
// A profile may narrow only issue typecheck/build. The checker, issue tests,
// merge smoke and full feature integration remain authoritative.
export function issueProjectChecks(manifest, files, interfaceChange = false) {
  const defaults = { typecheck: manifest?.verify?.typecheck, build: manifest?.verify?.build };
  if (interfaceChange || !files.length) return defaults;
  const code = files.filter(f => !docsOnly([f]));
  const profile = (manifest?.verify?.issue_profiles || []).find(p =>
    code.length > 0 && code.every(f => matchesAny(f, p.only)));
  if (!profile) return defaults;
  return Object.fromEntries(Object.entries(defaults).map(([key, command]) =>
    [key, Object.hasOwn(profile, key) ? profile[key] : command]));
}
...
export function finish(p, id) {
  ...
  const { manifest, errors } = readManifest(wt);
  if (errors.length) return red('manifest validation', errors);
  const prose = docsOnly(changed);
  const projectChecks = issueProjectChecks(manifest, changed, lease.interface_change);
  // lazy env: install once, only when a gate below actually needs a runtime in this worktree
  const runtimeCmds = [...(lease.verify_commands || []), ...(prose ? [] : [manifest?.checker?.command, projectChecks.typecheck, projectChecks.build])];
  ...
  for (const k of ['typecheck', 'build']) {
    const c = projectChecks[k];
    if (c === null) { steps.push({ step: k, skipped: 'scoped issue profile' }); continue; }
...
export function integrateFeature(p, fid, { resume = false } = {}) {
  ...
  for (const k of ['typecheck', 'build', 'test']) {
    const command = k === 'typecheck' ? (manifest?.verify?.typecheck_before_test || manifest?.verify?.typecheck) : manifest?.verify?.[k];
    if (command && !run('machine', command, k)) return { ok: false, reason: `${k} red`, steps };
  }
