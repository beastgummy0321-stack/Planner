// Queue scheduling and the /work lifecycle scripts: next, finish, merge, block, diff, integrate, plan-sync.
import fs from 'node:fs';
import path from 'node:path';
import { spawnSync } from 'node:child_process';
import { QUEUE_DIRS, readLease, writeLease, listLeases, leasePath, git, tryGit, gitRaw, parseFrontmatter, stringifyFrontmatter, matchesAny, norm, readState, writeState } from './core.mjs';
...
export function blockIssue(p, id, reason, evidence = [], steps = []) {
  ...
  const stamp = `\n\n## Blocked (${new Date().toISOString()})\n\nObserved: ${reason}\n\nEvidence:\n${evidence.map((e) => `- ${e}`).join('\n') || '- (none)'}\n\nWhy this issue cannot decide: needs the planner (re-slice, fix dependencies, settle the contract) or /grill.\n\nBoundary affected: see evidence.\n`;
  fs.writeFileSync(issueFile(p, 'blocked', id), text + stamp);
  if (where !== 'blocked') fs.unlinkSync(src);
  discardLease(p, id);
  return { ok: false, id, blocked: reason, evidence, steps };
}
...
export function integrateTicket(p, tid) {
  ...
  const steps = [];
  for (const c of t.data.verify || []) {
    const r = spawnSync(c, { cwd: p.root, shell: true, encoding: 'utf8' });
    steps.push({ step: c, ok: r.status === 0, tail: ((r.stdout || '') + (r.stderr || '')).trim().split('\n').slice(-20) });
    if (r.status !== 0) return { ok: false, reason: `integration verify red: ${c}`, steps };
  }
  const chk = runChecker(p.root);
  ...
  return { ok: true, ticket: tid, done, steps, next: `planner integration review, then harness close ticket ${tid}` };
}
...
export function integrateFeature(p, fid) {
  ...
  return { ok: true, feature: fid, tickets, steps, next: `acceptance against the confirmed direction, then harness close feature ${fid}` };
}
