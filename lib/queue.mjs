...
export function blockIssue(p, id, reason, evidence = [], steps = []) {
  ...
  writeJsonAtomic(path.join(p.harness, 'runtime', 'blocked', `${id}.json`), { fingerprint: lease?.issue_fingerprint || issueFingerprint(text, manifestText), reason, at: Date.now() });
  discardLease(p, id);
...
export function closeFeature(p, fid) {
  ...
  // discovery.md belongs to the NEXT feature (a /dig may precede closing the previous one); everything else was this feature's
  if (fs.existsSync(scratch)) for (const x of fs.readdirSync(scratch)) if (x !== 'discovery.md') fs.rmSync(path.join(scratch, x), { recursive: true, force: true });
