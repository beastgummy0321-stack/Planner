...
export function applyAdapter(project) {
  ...
  for (const cmd of plan.install) {
    ...
    if (r.status !== 0) throw new Error(`install failed: ${cmd}: ${r.stderr || r.stdout || ''}`);
  }
  if (plan.stack === 'ts') {
...
export function runChecker(root) {
  ...
  const r = runCommand(manifest.checker.command, root);
...
export function proveChecker(root) {
  ...
  try {
    ...
    for (const [rname, res] of Object.entries(manifest.resources || {})) {
      if (!['supabase-table','sql-table', ...(ext === 'ts' ? ['drizzle-table'] : ['sqlalchemy-model'])].includes(res.kind)) continue;
...
// Recognize common compound commands; arbitrary wrapper scripts prepare env explicitly.
const RUNTIME = /(?:^|&&|;|\|\||\n)\s*(?:npm|npx|pnpm|yarn|tsc|vitest|jest|playwright|uv|pytest)\b/i;
export function needsRuntime(cmd) {
  return RUNTIME.test(String(cmd || '')) || /(?:^|&&|;)\s*(?:node|python3?)\s+(?!-[ec]\b)\S+/i.test(String(cmd || ''));
}
export function envFingerprint(wt) {
  const hash = crypto.createHash('sha256');
  for (const dir of ['', ...jsSubprojects(wt)]) for (const f of ['package.json','package-lock.json','pnpm-lock.yaml','yarn.lock','.yarnrc.yml','pyproject.toml','uv.lock','requirements.txt']) {
    const file = path.join(wt, dir, f);
    if (fs.existsSync(file)) hash.update(dir + '/' + f).update(fs.readFileSync(file));
  }
  return hash.update(process.version).digest('hex');
}
export function envIsReady(lease) { return !!lease?.worktree && lease.env_ready === envFingerprint(lease.worktree); }
export function ensureEnv(project, lease) {
  if (!lease?.worktree) return { ok: false, log: [{ note: 'no worktree attached' }] };
  if (envIsReady(lease)) return { ok: true, log: [], cached: true };
  const { manifest } = readManifest(lease.worktree);
  const r = setupEnv(lease.worktree, manifest?.stack || detectStack(lease.worktree));
  if (r.ok) { lease.env_ready = envFingerprint(lease.worktree); writeLease(project, lease); }
  return r;
}
