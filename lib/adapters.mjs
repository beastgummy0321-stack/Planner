...
import { stringifyFrontmatter, readState, writeState, writeLease } from './core.mjs';
...
export function planAdapter(root) {
  ...
  if (!plan) throw new Error(`unsupported architecture adapter: stack ${stack}`);
...
export function envCommands(wt, stack) {
  ...
  if (stack === 'ts') {
    ...
    if (pm === 'pnpm') cmds.push('pnpm install --frozen-lockfile');
    else if (pm === 'yarn') cmds.push(yarnBerry(wt) ? 'yarn install --immutable' : 'yarn install --frozen-lockfile');
    else if (fs.existsSync(path.join(wt, 'package-lock.json'))) cmds.push('npm ci');
    else { cmds.push('npm install --no-package-lock'); notes.push('no lockfile: install is not reproducible; commit package-lock.json'); } // env setup never dirties the tree
