...
export function validateManifest(m, root = null, { materialized = false } = {}) {
  ...
  for (const k of ['test', 'typecheck', 'build', 'smoke', 'smoke_after_build']) if (m.verify && m.verify[k] !== undefined && typeof m.verify[k] !== 'string') e.push(`verify.${k} must be a command string`);
