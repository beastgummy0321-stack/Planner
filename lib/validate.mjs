...
export function validateManifest(m, root = null, { materialized = false } = {}) {
  ...
  for (const k of ['test', 'typecheck', 'typecheck_before_test', 'build', 'smoke', 'smoke_after_build']) if (m.verify && m.verify[k] !== undefined && typeof m.verify[k] !== 'string') e.push(`verify.${k} must be a command string`);
  if (m.verify?.typecheck_before_test && (!m.verify?.typecheck || !m.verify?.test)) e.push('verify.typecheck_before_test requires both typecheck and test');
  if (m.verify?.issue_profiles !== undefined) {
    const profiles = m.verify.issue_profiles;
    if (!Array.isArray(profiles) || !profiles.length) e.push('verify.issue_profiles must be a non-empty list');
    else for (const p of profiles) {
      if (!p || typeof p !== 'object' || Array.isArray(p)) { e.push('invalid issue profile'); continue; }
      if (Object.keys(p).some(k => !['only', 'typecheck', 'build'].includes(k))) e.push('issue profile supports only only/typecheck/build');
      if (!Array.isArray(p.only) || !p.only.length || p.only.some(g => typeof g !== 'string' || !g ||
        path.posix.isAbsolute(g) || path.win32.isAbsolute(g) || g.includes('\\') || g.split('/').includes('..')))
        e.push('issue profile only must contain project-relative globs');
      if (!Object.hasOwn(p, 'typecheck') && !Object.hasOwn(p, 'build')) e.push('issue profile needs a check override');
      for (const key of ['typecheck', 'build']) if (Object.hasOwn(p, key) && p[key] !== null && (typeof p[key] !== 'string' || !p[key].trim()))
        e.push(`issue profile ${key} must be a non-empty command or explicit null`);
    }
  }
  if (m.verify?.smoke_after_build && (!m.verify?.build || !m.verify?.smoke)) e.push('verify.smoke_after_build requires both build and smoke');
  if (m.verify?.build_outputs !== undefined) {
    const outputs = m.verify.build_outputs;
    if (!m.verify.build || !Array.isArray(outputs) || !outputs.length || outputs.some(p =>
      typeof p !== 'string' || !p || path.posix.isAbsolute(p) || path.win32.isAbsolute(p) ||
      p.split(/[\\/]/).some(s => !s || s === '.' || s === '..' || s === '.git' || s === '.harness' || s === '.work') || /[*?\[\]]/.test(p)))
      e.push('verify.build_outputs requires build and a non-empty list of relative output paths (no traversal, runtime state or globs)');
  }
...
export function validateIssueText(text, ctx = {}) {
  ...
  if (touchesDeps && !Array.isArray(d.deps)) e.push('touching dependency files requires deps: [] or the packages actually changed');
  const body = fm.body;
